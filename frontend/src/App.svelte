...
<main>
	...
	<p>Denne tjenesten lagrer <strong>ikke</strong> bilder eller noe informasjon om deg ❤️</p>
	<small>Og ja, den finner for mange feil bilder, men heller det enn motsatt.</small>
	...
	<div id="bilde">
		<h2>Velg bilde</h2>
		{#if image}
			<img class="avatar" src="{image}" alt="d" on:click={()=>{fileinput.click();}}/>
		{:else}
	...
	<div id="sensitivitet"> 
		...
		<div class="slider">
			<Slider
				style="--mdc-theme-primary: #{(Math.round(value*3)).toString(16).padStart(2, '0') + '00' + (255-Math.round(value*3)).toString(16).padStart(2, '0')};"
				bind:value
				min={20}
				max={65}
				step={0.1} 
	...
	<div id="send"> 
		<h2> Kjør </h2>
		<p> TODO åpne i new tab (Tillater internbilder) eller åpne alle offentlige bilder her</p>
		<Button on:click={() => send()} variant="raised" style="background-color: #ff3e00">
			<Label>Kjør</Label>
		</Button>

		{#if loading === true}
			<CircularProgress style="height: 32px; width: 32px;" indeterminate />
			<p> VIKTIG: Klikk på bildet for å få fg sin side der du kan laste ned full-versjonen!
		{/if}
	...
	<div id="result">
		{#if imageDatas !== undefined}
			...
			{#each imageDatas as imageData}
				...
				<img style="max-width:90%;" src="https://fg.samfundet.no{imageData.thumb}" alt="{imageData.motive}"/>
			{/each}
		{/if}
	</div>
